...
const BACKGROUND_BOOL_KEYS = new Set([
  "backgroundanimated",
  "animatedbackground",
  "animatebackground",
  "backgroundanimation",
...
function collectTagsAny(value: any): string[] {
  if (typeof value === "string") {
    return splitTagString(value);
  }
  if (typeof value === "number" || typeof value === "boolean") {
    return splitTagString(String(value));
  }
  if (Array.isArray(value)) {
    const out: string[] = [];
    for (const entry of value) {
      out.push(...collectTagsAny(entry));
    }
    return out;
  }
...
function collectTagsFromObject(obj: any): string[] {
  if (Array.isArray(obj)) {
    const res: string[] = [];
    for (const entry of obj) {
      res.push(...collectTagsFromObject(entry));
    }
    return res;
  }
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (TAG_FIELD_HINTS.has(normKey)) {
      res.push(...collectTagsAny(value));
    } else if (isPlainObject(value) || Array.isArray(value)) {
      res.push(...collectTagsFromObject(value));
    }
  }
  return res;
}
...
function collectTagsFromElement(element: TemplateElement | undefined): string[] {
  if (!element) return [];
  const tags: string[] = [];
  if ((element as any)?.tag != null) {
    tags.push(...collectTagsAny((element as any).tag));
  }
  if ((element as any)?.tags != null) {
    tags.push(...collectTagsAny((element as any).tags));
  }
  if (Array.isArray((element as any)?.elements)) {
    for (const child of (element as any).elements as TemplateElement[]) {
      tags.push(...collectTagsFromElement(child));
    }
  }
  return tags;
}

function gatherSlideTagsFromMods(
  mods: Record<string, any>,
  slideName: string
): string[] {
  const collected: string[] = [];
  const normalizedSlide = slideName.toLowerCase();
  const normalizedSlideKey = normalizeKey(slideName);

  for (const [rawKey, rawVal] of Object.entries(mods)) {
    if (typeof rawKey !== "string") continue;
    const lowerKey = rawKey.toLowerCase();
    const normalizedKey = normalizeKey(rawKey);

    if (normalizedKey === normalizedSlideKey) {
      collected.push(...collectTagsFromObject(rawVal));
      continue;
    }

    if (lowerKey.startsWith(normalizedSlide + ".")) {
      const suffix = lowerKey.slice(normalizedSlide.length + 1);
      const segments = suffix.split(".");
      const hasTagSegment = segments.some((seg) =>
        TAG_FIELD_HINTS.has(normalizeKey(seg))
      );
      if (hasTagSegment) {
        collected.push(...collectTagsAny(rawVal));
      } else if (isPlainObject(rawVal) || Array.isArray(rawVal)) {
        collected.push(...collectTagsFromObject(rawVal));
      }
      continue;
    }

    if (normalizedKey.startsWith(normalizedSlideKey) && normalizedKey.length > normalizedSlideKey.length) {
      const suffix = normalizedKey.slice(normalizedSlideKey.length);
      if (TAG_FIELD_HINTS.has(suffix)) {
        collected.push(...collectTagsAny(rawVal));
      }
    }
  }
...
function findBackgroundBooleanInValue(value: any): boolean | undefined {
  ...
  if (Array.isArray(value)) {
    for (const entry of value) {
      const res = findBackgroundBooleanInValue(entry);
  ...
  for (const [key, val] of Object.entries(value)) {
    const normKey = normalizeKey(key);
    if (BACKGROUND_BOOL_KEYS.has(normKey)) {
      const parsed = findBackgroundBooleanInValue(val);
      if (parsed !== undefined) return parsed;
      continue;
    }
    if (
      normKey.includes("background") ||
      normKey.includes("animate") ||
      normKey.includes("kenburn") ||
      normKey.includes("zoom") ||
      normKey === "bg"
    ) {
      const nested = findBackgroundBooleanInValue(val);
...
function readBackgroundAnimationBooleanHint(
  mods: Record<string, any>,
  slideName: string
): boolean | undefined {
  const normalizedSlide = slideName.toLowerCase();
  ...
  for (const [rawKey, rawVal] of Object.entries(mods)) {
    ...
    if (normalizedKey === normalizedSlideKey) {
      const nested = findBackgroundBooleanInValue(rawVal);
      if (nested !== undefined) return nested;
      continue;
    }
    ...
    if (lowerKey.startsWith(normalizedSlide + ".")) {
      ...
      if (BACKGROUND_BOOL_KEYS.has(normSuffix)) {
        const parsed = findBackgroundBooleanInValue(rawVal);
        if (parsed !== undefined) return parsed;
      } else if (isPlainObject(rawVal) || Array.isArray(rawVal)) {
        const nested = findBackgroundBooleanInValue(rawVal);
        if (nested !== undefined) return nested;
    ...
    if (
      normalizedKey.startsWith(normalizedSlideKey) &&
      normalizedKey.length > normalizedSlideKey.length
    ) {
      const suffix = normalizedKey.slice(normalizedSlideKey.length);
      if (BACKGROUND_BOOL_KEYS.has(suffix)) {
        const parsed = findBackgroundBooleanInValue(rawVal);
        if (parsed !== undefined) return parsed;
      }
    }
...
function interpretBackgroundAnimationTags(
  tags: Iterable<string>
): "animate" | "static" | undefined {
  let animate = false;
  let disable = false;

  const backgroundHints = ["background", "backdrop", "bg", "sfondo"];
  const motionHints = [
    "animate",
    "animated",
    "animazione",
  ...
  for (const raw of tags) {
    ...
    if (disableHints.some((hint) => compact.includes(hint))) {
      disable = true;
      continue;
    }

    const hasBackground = backgroundHints.some((hint) => lower.includes(hint));
    const hasMotion =
      motionHints.some((hint) => lower.includes(hint)) ||
      compact.includes("kenburn") ||
      compact.includes("kenburns");

    if ((hasBackground && hasMotion) || (lower.includes("bg") && hasMotion)) {
      animate = true;
      continue;
    }
    if (compact.includes("kenburn")) {
      animate = true;
    }
  }

  if (disable) return "static";
  if (animate) return "animate";
  return undefined;
}

function resolveBackgroundAnimationHint(
  mods: Record<string, any>,
  comp: TemplateElement | undefined,
  slideName: string
): "animate" | "static" | undefined {
  const boolHint = readBackgroundAnimationBooleanHint(mods, slideName);
  if (boolHint !== undefined) {
    return boolHint ? "animate" : "static";
  }

  const tags = new Set<string>();
  collectTagsFromElement(comp).forEach((tag) => tags.add(tag));
  gatherSlideTagsFromMods(mods, slideName).forEach((tag) => tags.add(tag));

  if (!tags.size) return undefined;
  return interpretBackgroundAnimationTags(tags);
}
...
export function buildTimelineFromLayout(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const backgroundHint = resolveBackgroundAnimationHint(mods, comp, `Slide_${i}`);
    ...
    if (backgroundHint === "animate") {
      slide.backgroundAnimated = true;
    } else if (backgroundHint === "static") {
      slide.backgroundAnimated = false;
    } else if (isFillerSlide) {
      slide.backgroundAnimated = false;
