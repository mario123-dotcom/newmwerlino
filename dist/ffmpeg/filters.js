...
exports.shadeChain = shadeChain;
exports.zoomExprFullClip = zoomExprFullClip;
exports.buildFirstSlideTextChain = buildFirstSlideTextChain;
exports.buildRevealTextChain_XFADE = buildRevealTextChain_XFADE;
// src/ffmpeg/filters.ts
const autosize_1 = require("../utils/autosize");
const config_1 = require("../config");
/** Ombra laterale: matte RGBA */
function shadeChain(strength, gamma = 1.0, leftPower = 0.8, vertPower = 0.2, bias = 0.2) {
    if (strength <= 0)
        return "format=rgba,geq=r='0':g='0':b='0':a='0'";
...
function zoomExprFullClip(durationSec, fps) {
...
function djitter(i) { const x = Math.sin(i * 12.9898) * 43758.5453123; return x - Math.floor(x); }
...
function buildFirstSlideTextChain(txt, segDur, fontfile, videoW, videoH, fps, color = "black") {
    const orientation = (0, config_1.deriveOrientation)(videoW, videoH);
    // Forziamo lo stesso numero di righe per orientamento
    const fixedLines = orientation === "portrait" ? 4 : 3;
    const auto = (0, autosize_1.autosizeAndWrap)(txt, {
        orientation,
        isFirstSlide: true,
        videoW, videoH,
    ...
    if (!auto.lines.length || (auto.lines.length === 1 && auto.lines[0] === ""))
        return `[pre]null[v]`;
...
function buildRevealTextChain_XFADE(txt, segDur, fontfile, videoW, videoH, fps, color = "white", transition = "wipeup", _align = "center") {
    const orientation = (0, config_1.deriveOrientation)(videoW, videoH);
    const fixedLines = orientation === "portrait" ? 4 : 3;
    const auto = (0, autosize_1.autosizeAndWrap)(txt, {
        orientation,
